...
import { Check, Info, Flame, Apple, Printer, Copy, ChevronDown } from "lucide-react"
...
const ResultsDisplay = ({ results, ethnicityAdjustmentApplied }: ResultsDisplayProps) => {
  ...
  const handleCopy = async () => {
    const summary = [
      `MacroMentor Results`,
      `Goal: ${formatGoal(results.userProfile.goal)}`,
      `Daily Calories: ${results.calorieTarget} kcal`,
      ``,
      `Macros:`,
      `  Protein: ${round(results.macros.protein.grams)}g (${round(results.macros.protein.percentage)}%)`,
      `  Carbs: ${round(results.macros.carbs.grams)}g (${round(results.macros.carbs.percentage)}%)`,
      `  Fat: ${round(results.macros.fat.grams)}g (${round(results.macros.fat.percentage)}%)`,
      ``,
  ...
  return (
    <div className="p-6">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
        {/* 1. Target Calories Hero Card */}
        <motion.div variants={itemVariants}>
          <Card className="overflow-hidden border-2 border-primary/20 shadow-lg bg-gradient-to-r from-primary/10 to-primary/5 dark:from-primary/20 dark:to-primary/10">
            <CardContent className="pt-6 pb-6 text-center">
              <p className="text-sm text-muted-foreground mb-2">Your Daily Target</p>
              <div className="text-5xl md:text-6xl font-heading font-bold text-primary">
                {results.calorieTarget} <span className="text-2xl md:text-3xl">kcal</span>
              </div>
              <div className="flex items-center justify-center gap-3 mt-3">
                <span className={`text-sm font-medium ${results.deficitSurplus >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  {results.deficitSurplus >= 0 ? "+" : "-"}{Math.abs(round(results.deficitSurplus))} kcal {results.deficitSurplus >= 0 ? "surplus" : "deficit"}
                </span>
        ...
        {/* 3. Macronutrient Distribution Card */}
        <motion.div variants={itemVariants}>
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                {/* Recharts Donut Chart - Enlarged */}
                <div className="flex justify-center items-center md:col-span-1 h-64 w-full">
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  <div className="flex items-center">
                    <span className="h-3 w-3 rounded-full mr-2 flex-shrink-0" style={{ backgroundColor: 'var(--color-protein)' }}></span>
                    <div className="flex-1">
                      ...
                      <Progress value={results.macros.protein.percentage} className="h-2 mt-1" style={{ ['--progress-color' as string]: 'var(--color-protein)' }} />
                  ...
                  {/* Carbs */}
                  <div className="flex items-center">
                    <span className="h-3 w-3 rounded-full mr-2 flex-shrink-0" style={{ backgroundColor: 'var(--color-carbs)' }}></span>
                    <div className="flex-1">
                      <div className="flex justify-between items-baseline">
                      ...
                      <Progress value={results.macros.carbs.percentage} className="h-2 mt-1" style={{ ['--progress-color' as string]: 'var(--color-carbs)' }} />
                      <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {/* Fat */}
                  <div className="flex items-center">
                    <span className="h-3 w-3 rounded-full mr-2 flex-shrink-0" style={{ backgroundColor: 'var(--color-fat)' }}></span>
                    <div className="flex-1">
                      <div className="flex justify-between items-baseline">
                      ...
                      <Progress value={results.macros.fat.percentage} className="h-2 mt-1" style={{ ['--progress-color' as string]: 'var(--color-fat)' }} />
                      <p className="text-xs text-muted-foreground mt-1">
              ...
              <div className="bg-muted/50 p-4 rounded-lg border">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li><strong style={{ color: 'var(--color-protein)' }}>Protein:</strong> Builds and repairs muscle tissue, supports immune function, and increases satiety. Your target of {round(results.macros.protein.grams)}g ({round(results.macros.protein.grams / results.userProfile.weight, 1)}g/kg) is optimized for your {results.userProfile.goal === 'build_muscle' ? 'muscle building' : 'health'} goal.</li>
                  <li><strong style={{ color: 'var(--color-carbs)' }}>Carbs:</strong> Primary energy source, fuels brain and high-intensity exercise. Your target of {round(results.macros.carbs.grams)}g provides adequate energy while balancing your overall calorie goal.</li>
                  <li><strong style={{ color: 'var(--color-fat)' }}>Fat:</strong> Supports hormone production, brain health, and nutrient absorption. Your target of {round(results.macros.fat.grams)}g ({round(results.macros.fat.grams / results.userProfile.weight, 1)}g/kg) ensures adequate essential fatty acids.</li>
                </ul>
              </div>
            </CardContent>
          </Card>
        ...
        {/* 4. Collapsible: "How we calculated this" */}
        <motion.div variants={itemVariants}>
          <Collapsible open={calculationOpen} onOpenChange={setCalculationOpen}>
            <CollapsibleTrigger asChild>
              <Button variant="ghost" className="w-full justify-between p-4 h-auto border rounded-lg hover:bg-muted/50">
                <span className="font-medium">How we calculated this</span>
                <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${calculationOpen ? 'rotate-180' : ''}`} />
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent className="space-y-6 pt-4">
              {/* User Profile Grid */}
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-4 border rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card className="overflow-hidden border shadow-sm">
                  <CardHeader className="pb-2 bg-muted/30">
                    ...
                    <CardDescription>Calories burned at complete rest</CardDescription>
                  </CardHeader>
                ...
                <Card className="overflow-hidden border shadow-sm">
                  <CardHeader className="pb-2 bg-muted/30">
                    ...
                    <CardDescription>BMR + activity level</CardDescription>
                  </CardHeader>
                  <CardContent className="pt-4">
        ...
        {/* 5. Accordion for secondary content */}
        <motion.div variants={itemVariants}>
          <Accordion type="multiple" className="space-y-2">
            {/* Micronutrient Recommendations */}
            <AccordionItem value="micronutrients" className="border rounded-lg px-4">
              <AccordionTrigger className="hover:no-underline">
                <span className="font-medium">Micronutrient Recommendations</span>
              </AccordionTrigger>
              <AccordionContent className="space-y-4 pb-4">
                <p className="text-sm text-muted-foreground">Essential vitamins and minerals for optimal health</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Iron */}
            ...
            <AccordionItem value="recommendations" className="border rounded-lg px-4">
              ...
              <AccordionContent className="space-y-4 pb-4">
                <p className="text-sm text-muted-foreground">Optimize your nutrition plan for best results</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <AccordionItem value="meals" className="border rounded-lg px-4">
              <AccordionTrigger className="hover:no-underline">
                <span className="font-medium">Sample Daily Meal Structure</span>
              </AccordionTrigger>
